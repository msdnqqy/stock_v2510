...
def stress_test(run_name, input_token_len, n, output_token_len, swanlab_api_key):
    ...
    result = {}
    result['a'] = f"\nAverage First Token Time over {n} iterations: {avg_first_token_time:.4f} seconds"
    result['b']= f"Average Decode Time per Token over {n} iterations: {avg_decode_time:.4f} tokens/second"
    with open("deepseek_causal_results_load1.json", "w", encoding="utf-8") as f:
